...
public class UserDAOJDBC extends DataAccessObjectJDBC<User> {
    ...
    private static final String GET_LEADERBOARD = "SELECT login,score FROM score inner join user on user.id = score.id_user ORDER BY score DESC LIMIT 10";
        ...
        try (Connection connection = DriverManager.getConnection(dbUrl, dbLogin, dbPwd);
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    User user = new User();
                    user.setLogin(rs.getString("login"));
                    user.setBestScore(rs.getInt("score"));
